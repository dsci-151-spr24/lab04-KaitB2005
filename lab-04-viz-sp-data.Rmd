...
***There are `r nrow(dennys) rows` and `r ncol(dennys)` columns. Each row represents the data for La Quinta Inns. The variables are: 
`r names(laquinta)`.***
...
***There are now `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for on denny's restaraunt. The variables are: `r names(dennys)`. ***
...
La Quinta hotels are mostly located in the Americas but also have locations in other parts of the world. Dennys are located in the US.
...
***We can use the state variable to determine whether a location is in the US, specifically by comparing the state listed to a database of states in the U.S.***
...
```
***There are no Denny's locations outside of the U.S. in our data.***
